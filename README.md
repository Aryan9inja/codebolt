...
| Vector search | pgvector on PostgreSQL (planned)                                       |
| LLM provider  | OpenRouter (free tier), provider-agnostic interface                    |
| Metrics       | Prometheus                                                             |
...
**Locked model:** `poolside/laguna-m.1:free` via OpenRouter, selected after a larger reasoning model (Nemotron-3-Ultra-550B) proved too slow/throttled on the free tier. Debugging approach: isolate provider issues via direct curl before assuming an application bug — this resolved the Nemotron timeout confusion cleanly and is the pattern to reuse for future model swaps.
...
- **pgvector on PostgreSQL** — cross-PR pattern detection, e.g. surfacing "this exact pattern was flagged/fixed in PR #N before." Not yet wired in.
- **Per-repo `.codebolt.yml` config loader** — the dispatch-map architecture has been ready since Day 6; loading hasn't been built. Will likely extend to LLM pipeline config too (e.g. per-repo model override) once the provider selector exists.
- **LLM provider selector** — the `LLMProvider` interface already exists specifically so a second implementation (Gemini) can be dropped in without touching the pipeline or prompt logic. Not yet built.
